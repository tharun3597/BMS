...
def edit(request):
    if 'id' in request.session:
        faculty = Register.objects.get(pk=request.session['id'])
        if request.method == 'POST':
            faculty.username = request.POST['name']
            faculty.password = request.POST['password']
            faculty.email = request.POST['email']
            faculty.save()
            return render(request, 'faculty/edit.html',{'faculty':faculty,'error':'Edited'})
        else:
            return render(request, 'faculty/edit.html', {'faculty': faculty})
    else:
        return redirect('faculty:flogin')
